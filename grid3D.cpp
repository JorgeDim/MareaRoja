...
extern int Cara3D__draw_caraGL,PoligonoPlano__readArchivoPol__ifstream_grid3D;
...
extern std::map<std::string, int>  CuentaLlamadas;

extern GLuint textureID;
extern GLuint textureID2;
extern GLUI *glui;
extern GLUI_TextBox *glui_edittext;
extern int Etapa,DibujaCentroVel;
...
extern float stepf;
extern int Global_nia;
extern double FactorVel;
extern float factorVmin;
...
extern int MODO_CampoVelocidades2,VelocidadEnHexahedros,VelocidadEnVertex;

void filtracomentario2(ifstream &myfile)
{
	...
	int i;
	int DBG_Filtra=0;
	//La otra me parece muy complicada (Noviembre 2018)


	//Elimino los espacios en blanco y \n
	pos=myfile.tellg();
	myfile.get(ch);
	if (DBG_Filtra) cout<<"filtracomentario2: ch="<<ch<<"="<<(int)ch<<endl;
	while (ch==32 || ch==10) {
		pos=myfile.tellg();
		myfile.get(ch);
	}
	while (ch==35) { //char(35)=#
		//Elimino el resto de la linea
		i=0;
		while (ch!=10) {
			imprimir[i]=ch;i++;
			myfile.get(ch);
		...
		imprimir[i]=0;
		if (DBG_Filtra)cout<<"Comentario :"<<imprimir<<endl;
		pos=myfile.tellg();
		myfile.get(ch);
		if (DBG_Filtra)cout<<"filtracomentario2: ch="<<ch<<"="<<(int)ch<<endl;
		while (ch==32 || ch==10) {
	...
	if (DBG_Filtra)cout<<"filtracomentario2: myfile.seekg(pos="<<pos<<")"<<endl;
	myfile.seekg(pos);
}
...
const MatrizSym3x3& MatrizSym3x3::operator = ( const MatrizSym3x3 M2)
{
	CuentaLlamadas[__PRETTY_FUNCTION__+string(" @ ")+static_cast< std::ostringstream & >((std::ostringstream()<<__LINE__)).str()]++;

#if DBG2==1
	cout<<"MatrizSym3x3& MatrizSym3x3::operator = "<<endl;
	cout <<"M2=[ "<<M2.a<< " "<<M2.b<< " "<<M2.c<< ""<<endl;
	cout <<"     "<<M2.b<< " "<<M2.d<< " "<<M2.e<< ""<<endl;
	cout <<"     "<<M2.c<< " "<<M2.e<< " "<<M2.f<< "]"<<endl;
#endif
...
MatrizSym3x3::MatrizSym3x3 (const MatrizSym3x3 &M)
{
	CuentaLlamadas[__PRETTY_FUNCTION__+string(" @ ")+static_cast< std::ostringstream & >((std::ostringstream()<<__LINE__)).str()]++;

#if DBG2==1
	cout<<"MatrizSym3x3::MatrizSym3x3 (const MatrizSym3x3& M) "<<endl;
	cout <<"M=[ "<<M.a<< " "<<M.b<< " "<<M.c<< ""<<endl;
	cout <<"      "<<M.b<< " "<<M.d<< " "<<M.e<< ""<<endl;
	cout <<"      "<<M.c<< " "<<M.e<< " "<<M.f<< "]"<<endl;
#endif
...
MatrizSym3x3 MatrizSym3x3::inversa ()
{
	CuentaLlamadas[__PRETTY_FUNCTION__+string(" @ ")+static_cast< std::ostringstream & >((std::ostringstream()<<__LINE__)).str()]++;
	...
	cout << "MatrizSym3x3::inversa..273" <<endl;
#endif

	double det=a*d*f + b*e*c + b*c*e - (c*c*d + a*e*e + b*b*f);
...
#if DBG2==1
	cout <<"inv=[ "<<inv.a<< " "<<inv.b<< " "<<inv.c<< ""<<endl;
	cout <<"      "<<inv.b<< " "<<inv.d<< " "<<inv.e<< ""<<endl;
	cout <<"      "<<inv.c<< " "<<inv.e<< " "<<inv.f<< "]"<<endl;
...
const R3 MatrizSym3x3::operator*  ( const R3& L)
{
	CuentaLlamadas[__PRETTY_FUNCTION__+string(" @ ")+static_cast< std::ostringstream & >((std::ostringstream()<<__LINE__)).str()]++;
	...
	R3 AL;
#if DBG2==1
	cout << "MatrizSym3x3::operator*..254" <<endl;
	...
	AL.x = a*L.x + b*L.y + c*L.z ;
	AL.y = b*L.x + d*L.y + e*L.z ;
	AL.z = c*L.x + e*L.y + f*L.z ;

#if DBG2==1

	cout <<"AL = [ "<<AL.x<< " "<<AL.y<< " "<<AL.z<< "]'"<<endl;
	cout << "MatrizSym3x3::operator*..260" <<endl;
#endif
...
MatrizSym3x3::MatrizSym3x3(void) {
}

MatrizSym3x3::MatrizSym3x3(double alfa) {
	a=alfa;
	b=a;
	c=a;
	d=a;
...
void PoligonoPlano::readArchivoPoly(ifstream &myfile,grid3D *papaL) {

	int i,sizeL,iv,Npoligs1,Nholes0;
	papa=papaL;
	int DBG_Poly=0;

	PoligonoPlano__readArchivoPol__ifstream_grid3D++;
	nRandom = (   (double)rand() / ((double)(RAND_MAX)+(double)(1)) );

	if (DBG>1) cout<<"PoligonoPlano::read(ifstream &myfile) "<<endl;
	if (binario) {
		cout<<"PoligonoPlano::readArchivoPoly intenta leer en binario!!!!"<<endl;
		exit(1);
	} else {
		filtracomentario2(myfile);
		myfile  >>Npoligs1>>Nholes0>>iBC;
		papa->addBC(iBC);
		if (DBG_Poly) cout  <<Npoligs1<<Nholes0<<iBC<<endl;;
		filtracomentario2(myfile);

		myfile  >> sizeL; punto.resize(sizeL);
		if (DBG_Poly) cout<<"sizeL="<<sizeL<<endl;
		for (i=0;i<   punto.size();i++)  {
			myfile  >> iv;

			if (DBG_Poly) cout<<"iv="<<iv<<endl;

			punto[i].x=papa->v3D[iv-1].x;
			punto[i].y=papa->v3D[iv-1].y;
			punto[i].z=papa->v3D[iv-1].z;
		}
	}
...
void grid3D::CalculaNormalVertex() {
	if (DBG) cout<<"grid3D::CalculaNormalVertex()"<<endl;
	...
	if (DBG) cout<<"grid3D::CalculaNormalVertex(): nV3D="<<nV3D<<endl;
	for (i=0;i<Cara.size();i++) {
		if (Cara[i].nVolumenes>1) continue;
		for (j=0;j<Cara[i].nvCara;j++) {
			k=Cara[i].iv[j];
			if (DBG) cout<<"grid3D::CalculaNormalVertex(): k="<<k<<endl;
	...
	if (DBG) cout<<"grid3D::CalculaNormalVertex():END"<<endl;
}
...
void grid3D::readFACE(char *ifile_name) {
	CuentaLlamadas[__PRETTY_FUNCTION__+string(" @ ")+static_cast< std::ostringstream & >((std::ostringstream()<<__LINE__)).str()]++;

	ifstream myfile;
	int i,j,k,itmp,v0,v1,v2,iBC,HayBC;
	cout<<"grid3D::readFACE("<<ifile_name<<")"<<endl;
	...
	for (i=0;i<nCaras;i++)  {
		...
		Cara[j].papa=this;
		Cara[j].nVolumenes=1;
		Cara[j].nRandom=(   (double)rand() / ((double)(RAND_MAX)+(double)(1)) );;
		if (iBC==0)
			Cara[j].nVolumenes=2;
...
void grid3D::readMallaXML(char *ifile_name) {
	ifstream myfile;
	int i,j,k,itmp;
	cout<<"grid3D::readMallaXML("<<ifile_name<<")"<<endl;

	XMLNode xMainNode=XMLNode::openFileHelper(ifile_name,"dolfin");
	XMLNode MeshNode=xMainNode.getChildNode("mesh");
	XMLNode VerticesNode=MeshNode.getChildNode("vertices");
	nV3D=atoi(VerticesNode.getAttribute("size"));
	v3D.resize(nV3D);
	cout <<"nV3D="<<nV3D<<endl;

	for (i=0;i<nV3D;i++) {
		int ii=i;
		XMLNode VertexNode=VerticesNode.getChildNode("vertex",&ii);
		j=atoi(VertexNode.getAttribute("index"));
		v3D[j].no=j;
		v3D[j].x=atof(VertexNode.getAttribute("x"));
		v3D[j].y=atof(VertexNode.getAttribute("y"));
		v3D[j].z=atof(VertexNode.getAttribute("z"));
		if (0 && i%1000000==0)
			cout <<"j="<<j<<"["<<v3D[j].x<<","<<v3D[j].y<<","<<v3D[j].z<<"]"<<endl;
	}
	XMLNode CellsNode=MeshNode.getChildNode("cells");
	nTetrahedros=atoi(CellsNode.getAttribute("size"));
	cout <<"nTetrahedros="<<nTetrahedros<<endl;
	Tetrahedro.resize(nTetrahedros);

	for (i=0;i<nTetrahedros;i++) {
		int ii=i;
		XMLNode CellNode=CellsNode.getChildNode("tetrahedron",&ii);
		j=atoi(CellNode.getAttribute("index"));
		Tetrahedro[j].no=j;
		int v0=atoi(CellNode.getAttribute("v0"));
		int v1=atoi(CellNode.getAttribute("v1"));
		int v2=atoi(CellNode.getAttribute("v2"));
		int v3=atoi(CellNode.getAttribute("v3"));
		if (0 && i%100000==0)
			cout <<"j="<<j<<"["<<v0<<","<<v1<<","<<v2<<","<<v3<<"]"<<endl;
		Tetrahedro[j].iv[0]=v0;
		Tetrahedro[j].iv[1]=v1;
		Tetrahedro[j].iv[2]=v2;
		Tetrahedro[j].iv[3]=v3;
	}

}
void grid3D::readPOLY(char *ifile_name) {
	ifstream myfile;
	int i,itmp;


	cout<<"grid3D::readPOLY("<<ifile_name<<")"<<endl;

	myfile.open (ifile_name);
	filtracomentario2(myfile);

	int Ndim,NAtributos,HayBC;
	myfile	>>nV3D	>>Ndim  >>NAtributos>>HayBC;
	cout<< "nV3D="<<nV3D<<"Ndim="<<Ndim <<"NAtributos="<<NAtributos<<"HayBC="<<HayBC<<endl;
	...
	filtracomentario2(myfile);
	myfile	>>nPoligonos>>HayBC;

	Poligono.resize(nPoligonos);

	for (i=0;i<nPoligonos;i++) {
		Poligono[i].readArchivoPoly(myfile,this);
	}

	//Transformo iBC en posicion relativa (podría ser otra variable)
	for (i=0;i<nPoligonos;i++) {
		Poligono[i].iBC=addBC(Poligono[i].iBC);
	}
}
...
void grid3D::readMSH3D(char *ifile_name) {
	...
	if (PrintLinea) cout<<__LINE__<<": grid3D::readMSH3D(): ifile_name="<<ifile_name<<endl;
...
void grid3D::read(ifstream &myfile)
{
	...
	if (binario) {
		...
		cout<<"Version Binaria="<<Version<<endl;

		myfile.read((char*)&i,sizeof(nH3D)*5);
		if (DBG) cout<<"i="<<i<<endl;
		nH3D=i;
		if (DBG) cout<<"nH3D="<<nH3D<<endl;
		myfile.read((char*)&xmin,sizeof(xmin)*6);
		if (DBG) cout<<"xmin="<<xmin<<endl;

		if (DBG) cout<<"1"<<endl;
		myfile.read((char*)&sizeL,sizeof(sizeL));   	v3D.resize(sizeL);
		nV3D=sizeL;

		if (DBG) cout<<"nV3D="<<nV3D<<endl;
		...
		nCaras=sizeL;
		if (DBG) cout<<"nCaras="<<nCaras<<endl;
		...
		char s[100];
		myfile  >>s>>Version;
		if (PrintLinea) cout <<"Version texto="<<Version<<endl;
...
void Cara3D::draw_caraGL00(vector<double> &F,double minF,double maxF,int* ii)
{
	...
	if (DBG2) cout<<__FUNCTION__<<"@"<<__LINE__<<" "<<endl;
	if (papa->DrawBC[papa->lBC[iBC]]==0) {
		return;
	}
...
void grid3D::print_poligonos() {
	CuentaLlamadas[__PRETTY_FUNCTION__+string(" @ ")+static_cast< std::ostringstream & >((std::ostringstream()<<__LINE__)).str()]++;

	int i,j;
	for (i=0;i<nPoligonos;i++) {
		cout<<"Poligono "<<i<<endl;
		cout<<"nPuntos "<<Poligono[i].nPuntos<<endl;
		for (j=0;j<Poligono[i].nPuntos;j++) {
			cout <<"["<<Poligono[i].punto[j].x<<","<<Poligono[i].punto[j].y<<","<<Poligono[i].punto[j].z<<"]"<<endl;
		}

	}
}

int grid3D::addBC(int iBC){
	//BC: arreglo ordenado con las diferentes iBC (ej: [0,5,10,11])
	//DrawBC: arreglo de 1 o 0, que indica si se dibuja o no la iBC. (ej: [1,1,0,0])
	//lBC:	  arreglo de largo=max(iBC)+1, que indica para cada iBC, su ubicación en BC
	//        es decir BC[lBC[iBC]]=iBC, lBC[BC[i]]=i
	//        ej:[0,0,0,0,0,1,0.....,2,3]
	int i;
	for (i=0;i<nBC;i++) {
		if (iBC>BC[i]) continue;
		if (iBC==BC[i]) return(i);
		if (iBC<BC[i]) break;
	}
	BC.insert( BC.begin()+i , iBC);
	nBC=BC.size();
	DrawBC.insert( DrawBC.begin()+i , 1);
	if (iBC+1>lBC.size()) {
		lBC.resize(iBC+1);
	}
	for (i=0;i<nBC;i++) {
...
void grid3D::Poligono_Inicial(R3 a, R3 b, PoligonoPlano &P)
{
	...
	double d2x,d2y,d2z,dd;
	double d3x,d3y,d3z;
	double lambda=(xmax+ymax+zmax-(xmin+ymin+zmin))/30;
	d1x=(b.x-a.x); d1y=(b.y-a.y); d1z=(b.z-a.z);
	dab=sqrt(sqr(d1x)+sqr(d1y)+sqr(d1z));
	if (fabs(d1x)>dab/4) {
		d2x=-d1y; d2y=d1x; d2z=0;
...
void grid3D::CentroCarasBloques()
{
	...
	for(i=0;i<nTriPrisma3D;i++) {
		...
		for(j=0;j<6;j++) {
			...
			if (i==0) {
				cout <<"z"<<j<<"="<<v3D[TriPrisma3D[i].iv[j]].z<<endl;
			}
		}
		if (i==0) {
			cout <<"6*zg="<<zg<<endl;
...
//	nVolFinito=1;
	...
	for(i=0;i<nVolFinito;i++) {
		if (i<nH3D) {
			...
			imprimir=0;
			if (i==0 ) imprimir=1;
			if (i==21272 ) imprimir=1;
			if (i==23432 ) imprimir=1;

			if (imprimir) {
				cout<<endl<<__LINE__<<": VolFinito="<<i<<endl;
			}
			...
			for (j=0;j<5;j++) {
				...
				if (imprimir) {
					cout<<__LINE__<<": jTP="<<jTP<<"  icj="<<icj<<endl;
				}
				if (Cara[icj].nVolumenes>1) {
					VolFinito[i].tipo_vecino[j]=ES_BLOQUE;
					ibb=Cara[icj].ih[0]; if (ibb==i) ibb=Cara[icj].ih[1];
					...
					if (imprimir) {
						cout<<__LINE__<<":   VolFinito[i].vecino["<<j<<"]=BLOQUE:"<<ibb<<endl;
						cout<<__LINE__<<":   Normal=["<<VolFinito[i].vecino_normal[j].x<<","
								<<VolFinito[i].vecino_normal[j].y<<","<<VolFinito[i].vecino_normal[j].z<<"]"<<endl;
						cout<<__LINE__<<":   TriPrisma3D["<<jTP<<"].centro=["<<TriPrisma3D[jTP].centro.x<<","
								<<TriPrisma3D[jTP].centro.y<<","<<TriPrisma3D[jTP].centro.z<<"]"<<endl;
						cout<<__LINE__<<":   TriPrisma3D["<<ibb<<"].centro=["<<TriPrisma3D[ibb].centro.x<<","
								<<TriPrisma3D[ibb].centro.y<<","<<TriPrisma3D[ibb].centro.z<<"]"<<endl;
						cout<<__LINE__<<":   TriPrisma3D["<<ibb+1<<"].centro=["<<TriPrisma3D[ibb+1].centro.x<<","
								<<TriPrisma3D[ibb+1].centro.y<<","<<TriPrisma3D[ibb+1].centro.z<<"]"<<endl;
						cout<<__LINE__<<":   TriPrisma3D["<<ibb+2<<"].centro=["<<TriPrisma3D[ibb+2].centro.x<<","
								<<TriPrisma3D[ibb+2].centro.y<<","<<TriPrisma3D[ibb+2].centro.z<<"]"<<endl;
					}
				} else {
					VolFinito[i].tipo_vecino[j]=ES_CARA;;
					VolFinito[i].vecino[j]=icj;
					...
					VolFinito[i].vecino_normal[j]=Cara[icj].normalCara;
					if (imprimir) {
						cout<<__LINE__<<":   VolFinito[i].vecino["<<j<<"]=CARA:"<<icj<<endl;
						cout<<__LINE__<<":   Normal=["<<VolFinito[i].vecino_normal[j].x<<","
								<<VolFinito[i].vecino_normal[j].y<<","<<VolFinito[i].vecino_normal[j].z<<"]"<<endl;
					}
				}
	...
	if(1) {
		...
		if (1+PrintVecinos1)  cout<<"P soporte de mis vertices:"<<endl;tic();
		int navance=0;
		for(i=0;i<nVolFinito;i++) {
			...
			for (j=0 ; j<nvertices ;j++) {
				...
				if (imprimir==1) {
					cout << __LINE__<<": "<<i<<" Tiene v3D[vertice].soporte.size()="<<v3D[vertice].soporte.size()<<endl;
				}
				for (int ell=0;ell<v3D[vertice].soporte.size();ell++) {
					...
					if (l>=VolFinito[i].vecino.size()) {//es un nuevo vecino
						...
						VolFinito[i].vecino_normal[l].NormaUnitario();
						if (imprimir==1) {
							cout << __LINE__<<": "<<i<<" Tiene un Nuevo Vecino="<<ibb<<endl;
						}
					}
				}
			}
					...
					if (VolFinito[ibb].tipo_vecino[k]==ES_CARA ) {
						...
						if (l>=VolFinito[i].vecino.size()) { //es un nuevo vecino
							...
							NB=VolFinito[ibb].vecino_normal[k];
							{
								...
								if (imprimir) {
									cout<<__LINE__<<": Cara="<<VolFinito[ibb].vecino[k];
							...
							if (loagrego) {
								if (imprimir) {
									cout<<__LINE__<<": Agrego este vecino"<<endl;
								}
								VolFinito[i].vecino.push_back(VolFinito[ibb].vecino[k]);
...
void grid3D::GeneraCarasTetrahedros()
{
	CuentaLlamadas[__PRETTY_FUNCTION__+string(" @ ")+static_cast< std::ostringstream & >((std::ostringstream()<<__LINE__)).str()]++;

	int ib,i;

	cout<<"grid3D::GeneraCarasTetrahedros()"<<endl;
	int navance=0;
	for (i=TetrahedrosAnalizados;i<nTetrahedros;i++){

		if (i>=navance*(nTetrahedros-TetrahedrosAnalizados)/20) {
			printf(".%d",navance);fflush(stdout);
			myfileSalida<<"."<<navance;
			navance++;
		}
		...
		ib=i;
		Tetrahedros * TetraI= &(Tetrahedro[i]);

		if (i%100000==0) {
			cout<<"i="<<i<<endl;
			cout<<"vertices:"<<TetraI->no<<":"<<TetraI->iv[0]<<" "<<TetraI->iv[1]<<" "<<TetraI->iv[2]
																								   <<" "<<TetraI->iv[3]<<endl;
		}

		TetraI->icara[0]=AddCara3(i,TetraI->iv[0],TetraI->iv[2],TetraI->iv[1],0);
		TetraI->icara[1]=AddCara3(i,TetraI->iv[0],TetraI->iv[3],TetraI->iv[2],0);
		TetraI->icara[2]=AddCara3(i,TetraI->iv[0],TetraI->iv[1],TetraI->iv[3],0);
		TetraI->icara[3]=AddCara3(i,TetraI->iv[1],TetraI->iv[2],TetraI->iv[3],0);
	...
	cout<<".OK"<<endl;
	myfileSalida<<".OK"<<endl;
	TetrahedrosAnalizados=nTetrahedros;
	cout <<"nCaras="<<nCaras<<endl;
}
...
void grid3D::SeleccionaYMuestraCara(double px,double py,double pz)
{
	...
	for (i=0;i<DrawBC.size();i++) {
		cout << "DrawBC["<<i<<"]="<<DrawBC[i]<<endl;
	}
	for (i=0;i<lBC.size();i++) {
		cout << "lBC["<<i<<"]="<<lBC[i]<<endl;
	}

	cout<<"C Cara: "<<pjmin<<":"<<endl;
	cout<<"C Centro=["<<Cara[pjmin].centro.x<<","<<Cara[pjmin].centro.y<<","<<Cara[pjmin].centro.z<<endl;
	...
	cout<<"C iBC="<<Cara[pjmin].iBC<<endl;
...
void grid3D::SeleccionaYMuestraPolygono(double px,double py,double pz)
{
	int i,j,jj,k,nr=5,nz=3,ir,iz,pj,pjmin;
	float pd2min,pd2;

	pd2min=1e10;
	for (pj=0;pj<nPoligonos;pj++) {
		pd2=sqr(px-Poligono[pj].centro.x)+sqr(py-Poligono[pj].centro.y)+sqr(pz-Poligono[pj].centro.z);
		if (pd2<pd2min) {
			pd2min=pd2;
			pjmin=pj;
		}
	}
	cout<<"P #Poligono: "<<pjmin<<":"<<endl;
	cout<<"P normal=["<<Poligono[pjmin].normal.x<<","<<Poligono[pjmin].normal.y<<","<<Poligono[pjmin].normal.z<<"]"<<endl;
	cout<<"P signo2="<<Poligono[pjmin].signo2<<endl;
	cout<<"P P =["<<px<<","<<py<<","<<pz<<"]"<<endl;

	printf("P Win1 =[%.2f %.2f %.2f]\n",Poligono[pjmin].winX1,Poligono[pjmin].winY1,Poligono[pjmin].winZ1);
	printf("P Win2 =[%.2f %.2f %.2f]\n",Poligono[pjmin].winX2,Poligono[pjmin].winY2,Poligono[pjmin].winZ2);
	printf("P Win3 =[%.2f %.2f %.2f]\n",Poligono[pjmin].winX3,Poligono[pjmin].winY3,Poligono[pjmin].winZ3);

	for (i=0;i<Poligono[pjmin].nPuntos;i++) {
		cout <<"P P("<<i<<")= ";
		printf("[%.2f %.2f %.2f]\n",Poligono[pjmin].punto[i].x,Poligono[pjmin].punto[i].y,Poligono[pjmin].punto[i].z);
	}
	cout<<endl;
...
void grid3D::Particulas_Init(int i)
{
	...
	if (DBG) cout <<"grid3D::Particulas_Init"<<endl;
	double x,y,z,t;
	int j,k;
...
void grid3D::drawVelGL(vector<double> UDraw,vector<double> VDraw,vector<double> WDraw)
/////OLD..BORRAR
{
	CuentaLlamadas[__PRETTY_FUNCTION__+string(" @ ")+static_cast< std::ostringstream & >((std::ostringstream()<<__LINE__)).str()]++;
	...
	for (i=0;i<nParticulas;i++) {
		...
		double UUU=sqrt(sqr(UDraw[jv3DMasCercano])+sqr(VDraw[jv3DMasCercano])+sqr(WDraw[jv3DMasCercano]));
		dx=UDraw[jv3DMasCercano]*dt/npasadas*factorV/UUU*factorDespegaLineas;
		dy=VDraw[jv3DMasCercano]*dt/npasadas*factorV/UUU*factorDespegaLineas;
		dz=WDraw[jv3DMasCercano]*dt/npasadas*factorV/UUU;
...
int grid3D::EnCualTriprisma(float px,float py,float pz,int tri0)
{
	...
	if (DBG)	cout<<"grid3D::EnCualTriprisma("<< px<<","<< py<<","<<pz<<","<<tri0<<")  __LINE__="<<__LINE__<<endl;
	...
	if (DBG)	cout<<"grid3D::EnCualTriprisma():END pjmin="<< pjmin<<")  __LINE__="<<__LINE__<<endl;
	return(pjmin);
}
...
void grid3D::drawParticulas_TriPrisma()
{
	...
	if (DBG)	cout<<"void grid3D::drawParticulas_TriPrisma():"<<endl;
	if (DBG)    cout <<"primerdrawVelGL="<<primerdrawVelGL<<" nParticulas="<<nParticulas<<endl;
	static double dt,dtCola;
	static int ipasadas=0;
	int i,j,k;
	double d2,d2min,x,y,z,dx,dy,dz,t;
	if(DBG) cout<<"grid3D::drawParticulas_TriPrisma():__LINE__= "<<__LINE__<<endl;
	if (primerdrawVelGL) {
		if (DBG)	cout<<"if (primerdrawVelGL): ok"<<endl;
		int oldsize=Particulas[0][0].size();
		...
		if (DBG)	cout<<"primerdrawVelGL-2"<<endl;
		for (j=0;j<3;j++) {
			Particulas0[j].resize(nParticulas);
		}
		if (DBG)	cout<<"primerdrawVelGL-3"<<endl;
		for (i=oldsize;i<nParticulas;i++) {
			ParticulaEsAleatoria[i]=true;
			Particulas_Init(i);
		}
		if (DBG)	cout<<"primerdrawVelGL-4"<<endl;
	...
	primerdrawVelGL=0;
	if (DBG)	cout<<"MODO_Pausa="<<MODO_Pausa<<endl;
	if(DBG) cout<<"grid3D::drawParticulas_TriPrisma():  __LINE__= "<<__LINE__<<endl;
	if (!MODO_Pausa) {
...
#pragma omp parallel for num_threads(GL_threads)
...
#endif
				{
					...
					if(DBG>1) cout<<"grid3D::drawParticulas_TriPrisma(): pjmin="<<pjmin<<"  __LINE__= "<<__LINE__<<endl;
					...
					if(DBG>0) cout<<"grid3D::drawParticulas_TriPrisma(): 3076"<<endl;

					{
						//cout<<"factorV="<<factorV<<endl;
						double UBp,VBp,WBp,UC;

						if(DBG>0) cout<<"grid3D::drawParticulas_TriPrisma(): __LINE__= "<<__LINE__<<endl;

						ParticulasBloq[i]=pjmin;

						if(DBG>0) cout<<"grid3D::drawParticulas_TriPrisma(): __LINE__= "<<__LINE__<<endl;

						if(DBG>0) cout<<"U.size="<<U.size()<<" step_hora="<<step_hora<<" stepf="<<stepf<<endl;

						UBp=U[step_hora][pjmin]*(1-stepf)+U[step_hora+1][pjmin]*stepf;
						VBp=V[step_hora][pjmin]*(1-stepf)+V[step_hora+1][pjmin]*stepf;
						WBp=W[step_hora][pjmin]*(1-stepf)+W[step_hora+1][pjmin]*stepf;
						UC=sqrt(sqr(UBp)+sqr(VBp)+sqr(WBp));
...
void grid3D::drawVelGL2()
{
	CuentaLlamadas[__PRETTY_FUNCTION__+string(" @ ")+static_cast< std::ostringstream & >((std::ostringstream()<<__LINE__)).str()]++;

	double x,y,z,x0,y0,z0,xf,yf,zf,vx,vy,vz,vmodulo;
	int ia,nia;
	if (DBG) cout<<"grid3D::drawVelGL2(vector<double> UU,vector<double> VV,vector<double> WW)"<<endl;
	...
	totalD = ((double)(clock())) / CLOCKS_PER_SEC; //Tiempo en segundo, con decimales
	ia=totalD;    //Tiempo en seundos sindecimales
	//cout<<"total_t="<<totalD<<"  ia="<<ia<<endl;
	nia=Global_nia;
	ia=(totalD-ia)*nia; //Decimal*5 \in {0,...,4}

	//cout<<"totalD="<<totalD<<"  ia="<<ia<<endl;

	if (VelocidadEnHexahedros)
		for (i=0;i<nH3D;i++) {
			x=h3D[i].centro.x;
			y=h3D[i].centro.y;
			z=h3D[i].centro.z;
	...
	//	cout<<"grid3D::drawVelGL2: VelocidadEnVertex="<<VelocidadEnVertex<<endl;
	if (VelocidadEnVertex) {
		double vmodmax;
		if (DibujaCentroVel==1) {
			...
			for (i=0;i<nV3D;i++) {
				...
				glTranslatef(x,y,z); FuncionesOpenGL::esfera(10.2,2);
				glTranslatef(-x,-y,-z);
			}
			glDisable( GL_LIGHTING );
		}
		if (factorVmin>0) {
			...
			for (i=0;i<nV3D;i++) {
				vx=UU[i]*factorV*100;
				vy=VV[i]*factorV*100;
				vz=WW[i]*factorV*FactorZ*100;
				vmodulo=sqrt(sqr(vx)+sqr(vy)+sqr(vz));
				if (vmodulo>vmodmax) vmodmax=vmodulo;
			}
		}
		for (i=0;i<nV3D;i++) {
			x=v3D[i].x;
			...
			vz=WW[i]*factorV*FactorZ*100;
			if (factorVmin>0) {
				vmodulo=sqrt(sqr(vx)+sqr(vy)+sqr(vz))+1e-20;
				double factor=factorVmin*vmodmax/vmodulo+(1-factorVmin);
				vx=vx*factor;
				vy=vy*factor;
				vz=vz*factor;
			}
...
void grid3D::drawVoronoi()
{
	...
	if (1 || QuienGeneraPoligonos==2) {
		...
		for (i=0;i<nVolFinito;i++) {
			...
			xV=VolFinito[i].centro.x;
			yV=VolFinito[i].centro.y;
			zV=VolFinito[i].centro.z;

			if (ModoDibujaAlgunos & VolFinito[i].nRandom>(double)Algunos_Porcentaje/100) {
				//cout <<"VolFinito["<<i<<"].Poligono["<<k<<"].nRandom="<<VolFinito[i].Poligono[k].nRandom<<endl;
				continue;
			}

			{
				for (k=0; k<VolFinito[i].Poligono.size(); k++) {
					...
					if (0 & ModoDibujaAlgunos & VolFinito[i].Poligono[k].nRandom>(double)Algunos_Porcentaje/100) {
						//cout <<"VolFinito["<<i<<"].Poligono["<<k<<"].nRandom="<<VolFinito[i].Poligono[k].nRandom<<endl;
						continue;
					}
					...
					xg2=xg*FactorCercania+(1-FactorCercania)*xV;
					yg2=yg*FactorCercania+(1-FactorCercania)*yV;
					zg2=zg*FactorCercania+(1-FactorCercania)*zV;
					...
					if (1) {
						...
						for (j=2; j<Pto->size(); j++) {
							glBegin(GL_TRIANGLES );
							glNormal3d(nx , ny , nz );
							glVertex3d( ((*Pto)[  0].x)*(1-lambdaP)+xV*lambdaP, ((*Pto)[  0].y)*(1-lambdaP)+yV*lambdaP, (((*Pto)[  0].z)*(1-lambdaP)+zV*lambdaP)*FactorZ);
							glVertex3d( ((*Pto)[j-1].x)*(1-lambdaP)+xV*lambdaP, ((*Pto)[j-1].y)*(1-lambdaP)+yV*lambdaP, (((*Pto)[j-1].z)*(1-lambdaP)+zV*lambdaP)*FactorZ);
							glVertex3d( ((*Pto)[  j].x)*(1-lambdaP)+xV*lambdaP, ((*Pto)[  j].y)*(1-lambdaP)+yV*lambdaP, (((*Pto)[  j].z)*(1-lambdaP)+zV*lambdaP)*FactorZ);
							glEnd();
...
void PoligonoPlano::drawGL()
{
	if (ModoDibujaAlgunos && nRandom>(double)Algunos_Porcentaje/100) {
		return;
	}
	if (papa->DrawBC[iBC]==0) {
		return;
	}
	if (Modo_DibujaCentroCaras||MODO_NumeraH) {
		glTranslatef(centro.x,centro.y,centro.z);
		FuncionesOpenGL::material(2);   FuncionesOpenGL::esfera(0.0002/Escala,3);
		{
			if (MODO_NumeraH){
				char *p;
				char ss[100];
	...
	if (ModoDibujaFrontera&& iBC !=0) {
		...
		if (0 && iBC == IBC_SUP) {
			...
			FuncionesOpenGL::material(3);
			FuncionesOpenGL::ColorF4(iBC*1.0/papa->nBC,1);
			draw_caraGL();
		}
	}

	if (ModoDibujaInterior && iBC ==0) {
		FuncionesOpenGL::material(2);
		FuncionesOpenGL::ColorF4(iBC*1.0/papa->nBC,1);
		draw_caraGL();
	}
...
void Cara3D::drawGL()
{
	if (DBG2) cout<<"Cara3D::drawGL()"<<endl;
	if (ModoDibujaAlgunos && nRandom>(double)Algunos_Porcentaje/100) {
		//		cout<<"return"<<endl;
		return;
	}
	...
	FuncionesOpenGL::ColorF4(iBC*1.0/(papa->nBC-1),0);
	//cout<<"iBC*1.0/papa->nBC="<<iBC*1.0/(papa->nBC-1)<<endl;
...
void PoligonoPlano::draw_caraGL()
{
	CuentaLlamadas[__PRETTY_FUNCTION__+string(" @ ")+static_cast< std::ostringstream & >((std::ostringstream()<<__LINE__)).str()]++;

	int li,signo;
	double x[14],y[14],z[14],v[3],xg,yg,zg,nx,ny,nz,nxx,nyy,nzz,nnn;
	double Anx[14],Any[14],Anz[14];
	xg=0;yg=0;zg=0;

	nPuntos=punto.size();
	for (li=0;li<nPuntos;li++) {
		x[li]=punto[li].x;
		y[li]=punto[li].y;
		z[li]=punto[li].z*FactorZ;
		xg+=x[li]/nPuntos;		yg+=y[li]/nPuntos;		zg+=z[li]/nPuntos;
	}
	if (0*CentroCalculado) {
		xg= centro.x;
		yg= centro.y;
		zg= centro.z*FactorZ;
	}
	...
	if (nPuntos==3) {
		glBegin(GL_TRIANGLES);
		for (li=0;li<nPuntos;li++) {
			glNormal3d(Anx[li],Any[li],Anz[li]);
			glVertex3d(x[li], y[li],z[li]);
		}
		glEnd();
		...
		for (li=0;li<nPuntos;li++) {
			...
			glVertex3d(x[li], y[li],z[li]);
			if (li+1<nPuntos)
				glVertex3d(x[li+1], y[li+1],z[li+1]);
			else
...
void grid3D::drawGL()
{
	...
	if (DBG) cout<<"grid3D::drawGL(): TriPrisma3D[i].CentroDibujado=0; ok "<<endl;
	FuncionesOpenGL::ObtieneMatrices();
#if Version==1
	if (DBG) cout<<"grid3D::drawGL(): Version==1"<<endl;
	if (Modo_DibujaCentroBloques) {
		if (DBG) cout<<"grid3D::drawGL(): Modo_DibujaCentroBloques==True"<<endl;
		//cout<<"grid3D.cpp: 4684"<<endl;
		if (ModoDibujaAlgunos && nH3D>0 ) {
			...
			if(DBG) cout<<"grid3D.cpp: 4713"<<endl;
			for (i=0;i<nPoligonos;i++) {

				glTranslatef(Poligono[i].centro.x,Poligono[i].centro.y,Poligono[i].centro.z*FactorZ);
				FuncionesOpenGL::material(1);   FuncionesOpenGL::esfera(0.1/Escala,3);
				glTranslatef(-Poligono[i].centro.x,-Poligono[i].centro.y,-Poligono[i].centro.z*FactorZ);
			}
			//cout<<"grid3D.cpp: 4720"<<endl;
		}
		if (DBG) cout<<"grid3D::drawGL()1600"<<endl;
	}
	if (DBG) cout<<"grid3D::drawGL(): nv3D="<<nV3D
			<<"\t nH3D="<<nH3D
			<<"\t nCaras="<<nCaras
			<<"\t Cara.size()="<<Cara.size()
			<<endl;
	...
	nPoligonos=Poligono.size();
	for (i=0;i<nPoligonos;i++)  {
		if (Poligono[i].iBC == IBC_SUP)  continue;
		if (Poligono[i].iBC == IBC_INF)  continue;
		Poligono[i].drawGL();
	}
...
void grid3D::drawGL(vector<double> & F)
{
	...
	CuentaLlamadas["grid3D::drawGL(vector<double> & F)"]++;
